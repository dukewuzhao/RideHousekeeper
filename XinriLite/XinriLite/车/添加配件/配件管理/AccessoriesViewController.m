...
@property (nonatomic, strong) UIAlertView *delateAlertView;
...
- (void)refrshView{
    ...
    NSString *fuzzyQuerySql = [NSString stringWithFormat:@"SELECT * FROM periphera_modals WHERE type = '%zd' AND seq = '%zd' AND bikeid = '%zd'",3,0,self.deviceNum];
    NSMutableArray *modals = [LVFmdbTool queryPeripheraData:fuzzyQuerySql];
    
    NSString *fuzzyQuerySql2 = [NSString stringWithFormat:@"SELECT * FROM periphera_modals WHERE type = '%zd' AND seq = '%zd' AND bikeid = '%zd'",3,1,self.deviceNum];
    NSMutableArray *modals2 = [LVFmdbTool queryPeripheraData:fuzzyQuerySql2];
    
    NSString *QuerykeySql = [NSString stringWithFormat:@"SELECT * FROM periphera_modals WHERE type = '%zd' AND bikeid = '%zd'", 2,self.deviceNum];
    NSMutableArray *keymodals = [LVFmdbTool queryPeripheraData:QuerykeySql];
    blekeyNumber = keymodals.count;
    //blekeyNumber = 2;
    NSString *QuerybraceletSql = [NSString stringWithFormat:@"SELECT * FROM periphera_modals WHERE type = '%zd' AND bikeid = '%zd'", 5,self.deviceNum];
...
- (void)setupViews{
    ...
    UICollectionView *collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, navHeight +ScreenHeight*.315+45, ScreenWidth, ScreenHeight*.315+35) collectionViewLayout:layout];
    ...
    UILabel *prompt = [[UILabel alloc] init];
    prompt.frame = CGRectMake(10, collectionView.height - 40, ScreenWidth - 20, 40);
    prompt.textAlignment = NSTextAlignmentCenter;
    prompt.textColor = [QFTools colorWithHexString:@"#666666"];
    prompt.font = [UIFont systemFontOfSize:13];
    prompt.text = @"注:手机连接车辆且感应钥匙或手环在车辆感应范围内，可在首页侧滑栏查看其电量。";
    [prompt setNumberOfLines:0];
    NSMutableAttributedString *attributedString = [[NSMutableAttributedString alloc] initWithString:prompt.text];
    NSMutableParagraphStyle *paragraphStyle = [[NSMutableParagraphStyle alloc] init];
    [paragraphStyle setLineSpacing:1];//调整行间距
    ...
    prompt.attributedText = attributedString;
    [prompt setLineBreakMode:NSLineBreakByCharWrapping];
    [collectionView addSubview:prompt];
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSString *QuerykeySql = [NSString stringWithFormat:@"SELECT * FROM periphera_modals WHERE (type = '%zd' OR type = '%zd') AND bikeid = '%zd'", 2,5,self.deviceNum];
    NSMutableArray *keymodals = [LVFmdbTool queryPeripheraData:QuerykeySql];
...
- (void)didSelectedAtIndexPath:(NSInteger)selectTag :(NSInteger)deviceid{
    ...
    if (selectTag == 30) {
        ...
        self.delateAlertView =[[UIAlertView alloc]initWithTitle:@"提示" message:@"是否删除该钥匙" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
        self.delateAlertView.tag =9999;
        [self.delateAlertView show];
        
    }else if (selectTag == 31){
        
        self.delateAlertView = [[UIAlertView alloc]initWithTitle:@"提示" message:@"是否删除该钥匙" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
        self.delateAlertView.tag =10000;
        [self.delateAlertView show];
        
    }else if (selectTag == 32){
        
        self.delateAlertView =[[UIAlertView alloc]initWithTitle:@"提示" message:@"是否删除该智能配件" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
        self.delateAlertView.tag =10002;
        [self.delateAlertView show];
        
    }else if (selectTag == 33){
        
        self.delateAlertView =[[UIAlertView alloc]initWithTitle:@"提示" message:@"是否删除该智能配件" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
        self.delateAlertView.tag =10003;
        [self.delateAlertView show];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (alertView.tag == 9999) {
        ...
        if (buttonIndex != [alertView cancelButtonIndex]) {
            ...
            NSString *fuzzyQuerySql = [NSString stringWithFormat:@"SELECT * FROM periphera_modals WHERE type = '%zd' AND seq = '%zd' AND bikeid = '%zd'",3,1,self.deviceNum];
            NSMutableArray *modals = [LVFmdbTool queryPeripheraData:fuzzyQuerySql];
            PeripheralModel *permodel = modals.firstObject;
...
-(void)dealloc{
    ...
    if (self.delateAlertView) {
        [self.delateAlertView dismissWithClickedButtonIndex:0 animated:YES];
        self.delateAlertView = nil;
    }
