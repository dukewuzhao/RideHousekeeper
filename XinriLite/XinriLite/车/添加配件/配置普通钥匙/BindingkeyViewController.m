...
- (void)setupNavView{
    ...
    [self.navView.centerButton setTitle:NSLocalizedString(@"key_set", nil) forState:UIControlStateNormal];
    @weakify(self);
...
- (void)setupheadView{
    ...
    binding.text = NSLocalizedString(@"normal_key_content", nil);
    ...
    [nextBtn setTitle:NSLocalizedString(@"no_bing_now", nil) forState:UIControlStateNormal];
    [nextBtn setTitleColor:[QFTools colorWithHexString:NSLocalizedString(@"VCControlColor", nil)] forState:UIControlStateNormal];
        ...
        if ([[date substringWithRange:NSMakeRange(12, 6)] isEqualToString:keyvalue]) {
            ...
            if (bindingtime == 1) {
                ...
                if ([key1 isEqualToString:[date substringWithRange:NSMakeRange(12, 8)]]) {
                    [SVProgressHUD showSimpleText:NSLocalizedString(@"key_button_double", nil)];
            ...
            [SVProgressHUD showSimpleText:NSLocalizedString(@"no_one_key", nil)];
...
-(void)bindingSuccess{
    self.binding.text = NSLocalizedString(@"set_normal_key_success", nil);
    self.nextBtn.frame = CGRectMake(ScreenWidth/2 - 50, ScreenHeight - 60 , 100, 40);
    
    [self bindKey];
    [SVProgressHUD showSimpleText:NSLocalizedString(@"set_normal_key_success", nil)];

}

- (void)bindKey{
    
    NSString *fuzzyQuerySql = [NSString stringWithFormat:@"SELECT * FROM periphera_modals WHERE bikeid LIKE '%zd' AND type LIKE '%d' AND seq LIKE '%zd'",self.deviceNum,3,self.seq];
    NSMutableArray *modals = [LVFmdbTool queryPeripheraData:fuzzyQuerySql];
    if (modals.count > 0) {
        
        NSString *delateKeySql = [NSString stringWithFormat:@"DELETE FROM periphera_modals WHERE bikeid LIKE '%zd' AND type LIKE '%d' AND seq LIKE '%zd'",self.deviceNum,3,self.seq];
        [LVFmdbTool deletePeripheraData:delateKeySql];
    }
    
    PeripheralModel *pmodel = [PeripheralModel modalWith:self.deviceNum deviceid:self.seq + 10 type:3 seq:self.seq mac:@"" sn:@"R000000000" firmversion:@""];
    BOOL isInsertp = [LVFmdbTool insertDeviceModel:pmodel];
    
    if (isInsertp) {
        
        [self.nextBtn setTitle:NSLocalizedString(@"back_accessories_management", nil) forState:UIControlStateNormal];
        ...
        [SVProgressHUD showSimpleText:NSLocalizedString(@"set_normal_key_fail", nil)];
    }
    [self nextBtnClick];
}
