...
+ (void)initialize {
 ...
 //#warning 必须先打开数据库才能创建表。。。否则提示数据库没有打开
    ...
    [_fmdb executeUpdate:@"CREATE TABLE IF NOT EXISTS bike_modals(id INTEGER PRIMARY KEY, bikeid INTEGER NOT NULL,bikename TEXT,ownerflag INTEGER,hwversion TEXT,firmversion TEXT,keyversion TEXT,mac TEXT,mainpass TEXT ,password TEXT, bindedcount INTEGER,ownerphone TEXT, fp_func INTEGER,fp_conf_count INTEGER,tpm_func INTEGER, vibr_sens_func INTEGER);"];
    ...
    /*胎压监测开启否表*/
    [_fmdb executeUpdate:@"CREATE TABLE IF NOT EXISTS pressurelock_models(id INTEGER PRIMARY KEY,bikeid INTEGER NOT NULL,pressurelock INTEGER);"];
...
//    }
//
//    if (![_fmdb columnExists:@"fp_func" inTableWithName:@"bike_modals"]){
//
//        NSString *alertStr = [NSString stringWithFormat:@"ALTER TABLE %@ ADD %@ INTEGER",@"bike_modals",@"fp_func"];
//        [_fmdb executeUpdate:alertStr];
//    }
//
//    if (![_fmdb columnExists:@"vibr_sens_func" inTableWithName:@"bike_modals"]){
//
//        NSString *alertStr = [NSString stringWithFormat:@"ALTER TABLE %@ ADD %@ INTEGER",@"bike_modals",@"vibr_sens_func"];
//        [_fmdb executeUpdate:alertStr];
//    }
//
//    if (![_fmdb columnExists:@"tpm_func" inTableWithName:@"bike_modals"]){
//
//        NSString *alertStr = [NSString stringWithFormat:@"ALTER TABLE %@ ADD %@ INTEGER",@"bike_modals",@"tpm_func"];
//        [_fmdb executeUpdate:alertStr];
//    }
//    if (![_fmdb columnExists:@"fp_conf_count" inTableWithName:@"bike_modals"]){
//        NSString *alertStr = [NSString stringWithFormat:@"ALTER TABLE %@ ADD %@ INTEGER",@"bike_modals",@"fp_conf_count"];
//        [_fmdb executeUpdate:alertStr];
//    }
//    if (![_fmdb columnExists:@"speeding_alarm" inTableWithName:@"pressurelock_models"]){
//        NSString *alertStr = [NSString stringWithFormat:@"ALTER TABLE %@ ADD %@ INTEGER",@"pressurelock_models",@"speeding_alarm"];
//        [_fmdb executeUpdate:alertStr];
//    }
}
...
+ (BOOL)insertBikeModel:(BikeModel *)model{
    ...
    NSString *insertSql = [NSString stringWithFormat:@"INSERT INTO bike_modals(bikeid,bikename, ownerflag, hwversion,firmversion,keyversion,mac,mainpass,password,bindedcount,ownerphone,fp_func,fp_conf_count,tpm_func,vibr_sens_func) VALUES ('%zd','%@','%zd','%@','%@','%@', '%@','%@','%@', '%zd','%@', '%zd', '%zd','%zd','%zd');", model.bikeid,model.bikename, model.ownerflag, model.hwversion,model.firmversion,model.keyversion,model.mac,model.mainpass,model.password,model.bindedcount,model.ownerphone,model.fp_func,model.fp_conf_count,model.tpm_func,model.vibr_sens_func];
...
+ (NSMutableArray *)queryBikeData:(NSString *)querySql {
    ...
    while ([set next]) {
        ...
        BikeModel *modal = [BikeModel modalWith:bikeid.intValue bikename:bikename ownerflag:ownerflag.intValue hwversion:hwversion firmversion:firmversion keyversion:keyversion mac:mac mainpass:mainpass password:password bindedcount:bindedcount.intValue ownerphone:ownerphone fp_func:fp_func fp_conf_count:fp_conf_count tpm_func:tpm_func vibr_sens_func:vibr_sens_func];
...
+ (NSMutableArray *)queryPressureLockData:(NSString *)querySql{
    ...
    while ([set next]) {
        ...
        PressureLockModel *modal = [PressureLockModel modalWith:bikeid.intValue pressureLock:pressurelock speeding_alarm:speeding_alarm];
        [arrM addObject:modal];
    }
    return arrM;
}
