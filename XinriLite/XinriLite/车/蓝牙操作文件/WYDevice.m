...
@implementation WYDevice{
    ...
    int PERIPHERAL_PASSWORD_POST_WAIT_TIMES;
}
...
//    NSArray *deices=  [centralManager retrieveConnectedPeripheralsWithServices:[[NSArray alloc]initWithObjects:[CBUUID UUIDWithString:@"FFE1"], nil]];
//    NSLog(@"%@",deices);
...
-(void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error{
    if (!error){
        NSLog(@"收到设备%d数据：%@ uuid:%@",_tag,characteristic.value,characteristic.UUID.UUIDString);
...
//        else
        if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"2A26"]]){
            ...
            if ( [[[ConverUtil data2HexString:characteristic.value] substringWithRange:NSMakeRange(8, 4)] isEqualToString:@"1002"]) {
                _verificationPassword = YES;
            }
...
-(void)checkSuccess:(CBPeripheral *)peripheral{
    if( peripheral.state==CBPeripheralStateConnected){
        ...
        [self VerificationConnectionSuccess];
    }
}

-(void)VerificationConnectionSuccess {
    ...
    if (!_binding) {
        ...
        dispatch_after(popTime, dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            if (_verificationPassword) {
                _verificationPassword = NO;
                PERIPHERAL_PASSWORD_POST_WAIT_TIMES = 0;
            }else{
                ...
                PERIPHERAL_PASSWORD_POST_WAIT_TIMES++;
                if (PERIPHERAL_PASSWORD_POST_WAIT_TIMES > 5) {
                    NSLog(@">>>error： 第%d次等待peripheral密码验证失败，请检查蓝牙设备是否可用",PERIPHERAL_PASSWORD_POST_WAIT_TIMES);
                    return;
                }
                
                NSLog(@">>> 第%d次等待peripheralManager验证连接",PERIPHERAL_PASSWORD_POST_WAIT_TIMES);
                
                [self remove];
                NSString*deviceuuid=[USER_DEFAULTS stringForKey:Key_DeviceUUID];
                if (deviceuuid) {
                    [self retrievePeripheralWithUUID:deviceuuid];//导入外设 根据UUID
                    [self connect];
                }
            }
        });
