...
- (void)updateApp
{
    ...
    NSDictionary *parameters = @{@"platform":@"I", @"channel": @"SUNRA",@"version": currentVersion};
    ...
    [[HttpRequest sharedInstance] postWithURLString:URLString parameters:parameters success:^(id _Nullable dict) {
        ...
        if ([dict[@"status"] intValue] == 0) {
            ...
            if([currentVersion floatValue] < [appStoreVersion floatValue]){
                UIAlertView *alert=[[UIAlertView alloc]initWithTitle:[NSString stringWithFormat:NSLocalizedString(@"bike_check_update", nil),nowStoreVersion] message:nil delegate:self cancelButtonTitle:NSLocalizedString(@"cancel", nil) otherButtonTitles:NSLocalizedString(@"sure", nil), nil];
