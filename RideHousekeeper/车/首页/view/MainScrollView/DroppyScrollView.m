...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    //NSLog(@"%d",scrollView.contentOffset.x);
}
//开始拖拽的时候停止计时器
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView {
//    if (scrollView.contentOffset.x>0) {
//        [scrollView setContentOffset:CGPointMake(self.selectIndex*ScreenWidth, scrollView.contentOffset.y) animated:NO];
//    }
}
//结束拖拽的时候开始定时器
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    CustomBike *bike = [self.items objectAtIndex:index];
    if (self.scrollViewIndex) {
        self.scrollViewIndex(index,bike.bikeid);
    }
}
