...
-(void)animateTransition:(id < UIViewControllerContextTransitioning >)transitionContext{
    ...
    UIViewController<AnimationObjectProtocol> * fromVC = (UIViewController <AnimationObjectProtocol> * )[transitionContext viewControllerForKey:UITransitionContextFromViewControllerKey];
    UIViewController<AnimationObjectProtocol> * toVC = (UIViewController <AnimationObjectProtocol> * )[transitionContext viewControllerForKey:UITransitionContextToViewControllerKey];
...
//    //找出到页面4个角最长的半径
//    CGFloat r1 = (ScreenWidth-centerX)>centerX?(ScreenWidth-centerX):centerX;
//    CGFloat r2 = (ScreenWidth-centerY)>centerY?(ScreenWidth-centerY):centerY;
//    CGFloat radius = sqrt((r1 * r1) + (r2 * r2));
    ...
    CGRect nowViewRect = [[fromVC targetTransitionView] convertRect: [fromVC targetTransitionView].frame toView:fromVC.view];
    CGRect toViewRect = [[toVC targetTransitionView] convertRect:[toVC targetTransitionView].frame toView:toVC.view];
    UIBezierPath *smallCircleBP =  [UIBezierPath bezierPathWithRoundedRect:nowViewRect cornerRadius:10];
    UIBezierPath *bigCircleBP = [UIBezierPath bezierPathWithRoundedRect:toViewRect cornerRadius:10];
    //跳转到新视图
    ...
    //返回
        ...
        maskLayer.path = bigCircleBP.CGPath;//将它的 path 指定为最终的 path 来避免在动画完成后会回弹
