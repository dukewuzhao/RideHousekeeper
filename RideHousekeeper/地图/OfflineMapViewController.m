    ...
    if(tableView == groupTableView)
    {
        ...
        if(indexPath.section==0)
        {
            ...
            if (updateInfo.status==1) {
                ...
                [self setTextColor:cityname FontNumber:[UIFont systemFontOfSize:13] AndRange:NSMakeRange(contentStr.length - 6, 6) AndColor:[QFTools colorWithHexString:MainColor]];
                ...
                NSString *contentStr = cityname.text;
                [self setTextColor:cityname FontNumber:[UIFont systemFontOfSize:13] AndRange:NSMakeRange(contentStr.length - 5, 5) AndColor:[QFTools colorWithHexString:MainColor]];
            }else if (updateInfo.status==10){
                
                down.image = [UIImage imageNamed:@"stopdown"];
                cityname.text = [NSString stringWithFormat:@"%@ (离线包导入中)", item.cityName];
                NSString *contentStr = cityname.text;
                [self setTextColor:cityname FontNumber:[UIFont systemFontOfSize:13] AndRange:NSMakeRange(contentStr.length - 8, 8) AndColor:[QFTools colorWithHexString:MainColor]];
            }else {
                down.image = [UIImage imageNamed:@"downloading"];
        ...
        if (indexPath.section == 0) {
            ...
            if(_arraylocalDownLoadMapInfo!=nil)
            {
                ...
                if(item.update)
                {
                    ...
                    if (updateInfo.status==1) {
                        sizelabel.textColor = [QFTools colorWithHexString:MainColor];
