...
- (void)GenderArray{
    self.selectLb.text = @"选择震动灵敏度";
    [self.array addObject:@[@"低",@"中",@"高"]];
}
...
- (void)hideAnimation{
    ...
    if([self.delegate respondsToSelector:@selector(hidePickerSelectorView:)])
    {
        [self.delegate hidePickerSelectorView:0];
    }
