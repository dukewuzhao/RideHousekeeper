...
@property(nonatomic, strong) CFLableView *labelView;
...
- (CFLableView *)labelView {
    if (_labelView == nil) {
        _labelView = [[CFLableView alloc] init];
...
- (UIButton *)headView {
    if (!_headView) {
        _headView = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 40, 40)];
...
- (void)setUpAccessoryView {
    if ([_item isKindOfClass:[CFSettingArrowItem class]]) { // Cell右边箭头
        ...
        self.labelView.rightLabelFontColor = labelItem.rightLab_color?labelItem.rightLab_color:_rightLabelFontColor;
        self.selectionStyle = UITableViewCellSelectionStyleNone;
        ...
        self.labelarrowView.rightLabelFontColor = labelarrowItem.rightLab_color?labelarrowItem.rightLab_color:_rightLabelFontColor;
        self.selectionStyle = UITableViewCellSelectionStyleDefault;
        ...
        [self.headView setImage:[SGQRCodeObtain generateQRCodeWithData:iconItem.icon_right size:self.headView.width] forState:UIControlStateNormal];
        [self.headView addTarget:self action:@selector(headViewClick:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)headViewClick:(UIButton *)btn{

    if ([_item isKindOfClass:[CFSettingIconItem class]]){
        if (_item.btnOpration) {

            _item.btnOpration(btn);
        }
    }
}
...
-(void)setRightLabelFont:(UIFont *)rightLabelFont{
    ...
    self.labelView.rightLabelFont = rightLabelFont;
...
-(void)setRightLabelFontColor:(UIColor *)rightLabelFontColor{
    ...
    self.labelView.rightLabelFontColor = rightLabelFontColor;
