...
//#import "BikeDashBoardViewController.h"
...
- (instancetype)initWithSideVC:(UIViewController *)sideMenuVC currentVC:(UIViewController *)currentMainVC change:(BOOL)isChange
{
    UIViewController *rootVC = [UIApplication sharedApplication].delegate.window.rootViewController;
    if (rootVC && [rootVC isKindOfClass:[self class]] && !isChange) {
    ...
    if (self = [super init]) {
        ...
        _currentVCPanEnableRange = XYScreenWidth / 4;
        _isSide = YES;
...
- (BOOL)shouldAutorotate{
    return [self.currentNavController.topViewController shouldAutorotate];
}

- (UIInterfaceOrientationMask)supportedInterfaceOrientations
{
    return [self.currentNavController.topViewController supportedInterfaceOrientations];
}
...
//            //竖屏
//            [AppDelegate currentAppDelegate].mainController = [BikeViewController new];
...
//            UIViewController *tbc = [[XYSideViewController alloc] initWithSideVC:[SideMenuViewController new] currentVC:navOne change:YES];
...
//            //横屏
//            [AppDelegate currentAppDelegate].mainController = [BikeDashBoardViewController new];
//            BaseNavigationController *navOne = [[BaseNavigationController alloc] initWithRootViewController:[AppDelegate currentAppDelegate].mainController];
//            UIViewController *tbc = [[XYSideViewController alloc] initWithSideVC:[SideMenuViewController new] currentVC:navOne change:YES];
//            _isSide = NO;
//            dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)setUpViewControllers
{
    ...
    tapView.backgroundColor = [UIColor colorWithWhite:0 alpha:0.4];
...
- (void)closeSideVC
{
    ...
    [(SideMenuViewController *)_sideVC stopWeatherLocation];
}
...
- (void)openSideVC
{
    ...
    [(SideMenuViewController *)_sideVC startWeatherLocation];
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context
{
    if ([keyPath isEqualToString:@"center"]) {
        ...
        float viewpercent = point.x/(viewStartCenterPoint.x + _sideContentOffset);
        tapView.width = XYScreenWidth - point.x + viewStartCenterPoint.x;
        tapView.backgroundColor = [UIColor colorWithWhite:0 alpha:0.4 * viewpercent];
        if (point.x >viewStartCenterPoint.x+1 && point.x != XYSccreenHeight/2){
            tapView.hidden = NO;
        }else{
            tapView.hidden = YES;
        }
