...
- (void)setupMenus{
    ...
    listTableView.separatorColor = [QFTools colorWithHexString:SeparatorColor];
    ...
    addbike.layer.mask = [self.view UiviewRoundedRect:addbike.bounds byRoundingCorners:UIRectCornerAllCorners cornerRadii:CGSizeMake(8, 8)];
    [addbike setTitle:@"新增车辆" forState:UIControlStateNormal];
...
- (void)addbikeClicked{
    ...
    AddBikeViewController *selectVc = [AddBikeViewController new];
...
//        image.layer.cornerRadius = 15;
        ...
        image.layer.mask = [self.view UiviewRoundedRect:image.bounds byRoundingCorners:UIRectCornerAllCorners cornerRadii:CGSizeMake(15, 15)];
        ...
        if ([USER_DEFAULTS integerForKey:Key_BikeId] == [[bikearray objectAtIndex:indexPath.section] bikeid]){
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [USER_DEFAULTS setInteger:[[bikearray objectAtIndex:indexPath.section] bikeid] forKey:Key_BikeId];
    ...
    [[Manager shareManager] switchingVehicle:dict];
    [self.navigationController popViewControllerAnimated:YES];
}
